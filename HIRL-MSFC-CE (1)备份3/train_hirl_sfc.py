...
def main():
    ...
    env.reward_critic.set_training_phase(2, epoch=0)
        ...
        try:
            ...
            while not request_done and attempts_per_req < max_attempts:
                ...
                while not sub_task_done and sub_attempts < 10:
                    ...
                    step_count += 1
                    env.reward_critic.epoch = step_count
                    episode_steps += 1
    ...
    # 4. 导出 (注意 prefix 包含目录路径)
    env.metrics_logger.export_all(prefix=f"{output_dir}/train_{timestamp}")
