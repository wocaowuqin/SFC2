...
class SFC_HIRL_Env(gym.Env):
    ...
    def __init__(self, input_dir: Path, topo: np.ndarray, dc_nodes: List[int], capacities: Dict):
        ...
        # [新增] 备份策略统计计数器
        self.stats_total_subtasks = 0  # 总共处理的子任务(目的节点)数量
        self.stats_backup_activated = 0  # 专家失败，触发备份的次数
        self.stats_backup_succeeded = 0  # 备份策略成功找到解的次数

        # [新增] 初始化奖励函数
        self.reward_critic = RewardCritic(training_phase=1, epoch=0)
    ...
    def step_low_level(self, goal_dest_idx: int, low_level_action) -> Tuple[np.ndarray, float, bool, bool]:
        ...
        if isinstance(low_level_action, tuple) and len(low_level_action) == 3:
            ...
            placement = {}
        # [新增] 统计：每一次 step_low_level 都是一次子任务决策
        ...
        # 2. 如果失败，使用 BackupPolicy 兜底
        if not feasible:
            # 准备状态字典：将 numpy 数组转换为 dict {index: value}
            ...
            if plan and plan.get('feasible'):
                ...
                backup_type = plan.get('backup_type', 'unknown')
                logger.debug(f"Using backup strategy: {backup_type} for goal {goal_dest_idx}")
