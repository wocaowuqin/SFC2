...
class SFC_HIRL_Env(gym.Env):
    ...
    def __init__(self, input_dir: Path, topo: np.ndarray, dc_nodes: List[int], capacities: Dict):
        ...
        import pickle  # 记得在文件头部引入 pickle

        # ... (在 __init__ 方法内) ...

        # --- 修改开始: 加载 Python 生成的 .pkl 文件 ---
        try:
            # 1. 加载请求数据
            ...
            with open(req_path, 'rb') as f:
                # Python 生成的本身就是 list[dict]，不需要 parse_mat_request 解析
            ...
            # --- 修复点：将 'arrive_event' 转换为 'arrive' ---
            self.events = []
            for evt in raw_events:
                # 兼容处理：检查是否存在 arrive_event 或 arrive
                arrive_data = evt.get('arrive_event', evt.get('arrive', []))
                leave_data = evt.get('leave_event', evt.get('leave', []))

                # 确保是 numpy array 或 list，并统一键名
                self.events.append({
                    'arrive': np.array(arrive_data, dtype=int).flatten(),
                    'leave': np.array(leave_data, dtype=int).flatten()
                })
            # -----------------------------------------------
            # 注意: Python 生成的 event_list 结构可能已经是 list[dict]
            # 结构应为: [{'arrive_event': [id1, id2], 'leave_event': []}, ...]
            # 如果你的 Python 生成器是这么写的，那么直接赋值即可，不需要像 .mat 那样 flatten()
    ...
    # # 修复3: 实现真正的 Greedy Backup Plan 已更新为sfc_backup_system
